...
enum class STATUS
{
    UNRECOGNISEDARG = -390, ARGUMENTSOVERFLOW = 3, ARGUMENTSUNDERFLOW = -3, DIVBYZERO = -1, SUCCESS = 0, ISNOTARRAY = 1, UNDECLAREDID = 2, NOFILE = -2
};
/**
* Naming:
...
STATUS interpret(const std::ifstream& file, const std::string& file_name, const std::vector<std::string>& argv = {})
{
    ...
    while(!file.eof())
    {
        ...
        case INSTR::DIV:
            ...
            break;
       case INSTR::RDIV:
            if(instruction.size() > 1) return WrongArity(file_name, line_num, "rdiv", 0, instruction.size() - 1);
            if(std::any_cast<int>(stack.top()) == 0) return DivisionByZero(file_name, line_num);
            op1 = stack.top(); stack.pop(); op2 = stack.top(); stack.pop(); stack.push(std::any_cast<long double>(op2) / std::any_cast<long double>(op1));
            break;
       case INSTR::POW:
            if(instruction.size() > 1) return WrongArity(file_name, line_num, "pow", 0, instruction.size() - 1);
            ...
            break;
       case INSTR::RPOW:
            if(instruction.size() > 1) return WrongArity(file_name, line_num, "rpow", 0, instruction.size() - 1);
            ...
            break;
       case INSTR::SQUARE:
            if(instruction.size() > 1) return WrongArity(file_name, line_num, "square", 0, instruction.size() - 1);
            op1 = stack.top(); stack.pop(); stack.push(std::pow(std::any_cast<long double>(op1), 2));
            break;
       case INSTR::CUBE:
            if(instruction.size() > 1) return WrongArity(file_name, line_num, "cube", 0, instruction.size() - 1);
            op1 = stack.top(); stack.pop(); stack.push(std::pow(std::any_cast<long double>(op1), 3));
            break;
       case INSTR::ANDB:
            if(instruction.size() > 1) return WrongArity(file_name, line_num, "andb", 0, instruction.size() - 1);
            ...
            break;
       case INSTR::ORB:
            if(instruction.size() > 1) return WrongArity(file_name, line_num, "orb", 0, instruction.size() - 1);
            op1 = stack.top(); stack.pop(); op2 = stack.top(); stack.pop(); stack.push(std::any_cast<unsigned long long>(op1) | std::any_cast<unsigned long long>(op2));
            break;
       // TODO: Bitwise operations and other operations
       case INSTR::PRINTI:
            if(instruction.size() > 1) return WrongArity(file_name, line_num, "printi", 0, instruction.size() - 1);
            std::cout << std::any_cast<int>(stack.top());
            break;
       case INSTR::PRINTPI:
            if(instruction.size() > 1) return WrongArity(file_name, line_num, "printpi", 0, instruction.size() - 1);
            std::cout << std::any_cast<int>(stack.top()); stack.pop();
            break;
       case INSTR::SCANI:
            if(instruction.size() > 1) return WrongArity(file_name, line_num, "scani", 0, instruction.size() - 1);
            int x{}; std::cin >> x; stack.top() = x;
            break;
       case INSTR::IDXI:
            if(instruction.size() > 2) return WrongArity(file_name, line_num, "idxi", 1, instruction.size() - 1);
            if(!is_array<decltype(stack.top())>::value) return OperandIsNotArray(file_name, line_num, stack.top());
            op1 = instruction[1]; stack.push(std::any_cast<std::vector<std::any>>(stack.top())[std::any_cast<size_t>(op1)]);
       default:
...
int main(int argc, char** argv)
{
    if(argc == 2)
    {
        ...
        if(IsFile(std::string(argv[1]))) return static_cast<int>(interpret(infile, std::string(argv[1])));
        else return static_cast<int>(ExecuteCommand(std::string(argv[1])));
    }
    else if(argc > 2)
    {
